...
def props_ren(old_data):
    ...
    old_properties = dict(
        sorted(
            old_properties.items(),
            key=lambda item: item[1].get("property_claims_count", 0),
            reverse=REVERSE_PROPS,
        )
    )
    # ---
    for n, (p, p_old) in tqdm(
        enumerate(old_properties.items()),
        desc="Work on props:",
        total=len(old_properties),
    ):
        # ---
