...
# ---
read_dump_dir = Path(__file__).parent
# ---
# split after /dump
...
def save_dump(tab, _claims="claims"):
...
def compare_file_date(file_date):
...
def read_file():
def compare_file_date(file_date):
    with codecs.open(f"{read_dump_dir}/file_date.txt", "r", encoding='utf-8') as outfile:
        old_date = outfile.read()
    # ---
    print(f"file_date: {file_date}, old_date: {old_date}")
    # ---
...
limit = {1: 15000}
                ...
                # ---
                if len(claims) == 0:
                    dump_data['items_0_claims'] += 1
                else:
                    # ---
                    if len(claims) == 1:
                        dump_data['items_1_claims'] += 1
                    # ---
                    if "P31" not in claims:
                        dump_data['items_no_P31'] += 1
                    # ---
                    ...
                    for p in claims.keys():
                        data_type = claims[p][0].get("mainsnak", {}).get("datatype", '')
                        # ---
                        if data_type == "wikibase-item":
                            if p not in dump_data['properties']:
                                dump_data['properties'][p] = {
                                    "qids": {"others": 0},
                                    "lenth_of_usage": 0,
                                    "len_prop_claims": 0,
                                }
                            dump_data['properties'][p]["lenth_of_usage"] += 1
                            dump_data['all_claims_2020'] += len(claims[p])
                            # ---
                            for claim in claims[p]:
                                dump_data['properties'][p]["len_prop_claims"] += 1
                                # ---
                                ...
                                # ---
                                if entity_id:
                                    if entity_id not in dump_data['properties'][p]["qids"]:
                                        dump_data['properties'][p]["qids"][entity_id] = 1
                                    else:
                                        dump_data['properties'][p]["qids"][entity_id] += 1
                                # ---
                                del entity_id
            ...
            # ---
    # ---
    print(f"read all lines: {dump_data['done']}")
    # ---
    for x, xx in dump_data['properties'].copy().items():
        dump_data['properties'][x]["len_of_qids"] = len(xx["qids"])
        # dump_data['properties'][x]["qids"] = {k: v for k, v in sorted(xx['qids'].items(), key=lambda item: item[1], reverse=True)}
    # ---
    dump_data['len_all_props'] = len(dump_data['properties'])
    # ---
    end = time.time()
    # ---
    delta = int(end - time_start)
    dump_data['delta'] = f'{delta:,}'
    # ---
    save_dump(dump_data)
    # ---
    with codecs.open(f"{read_dump_dir}/file_date.txt", "w", encoding='utf-8') as outfile:
        outfile.write(dump_data['file_date'])
    # ---
...
def process_dump_file():
    print(f"read file: {dump_file_path}")

    if not os.path.isfile(dump_file_path):
        print(f"file {dump_file_path} not found")
        return {}

    t1 = time.time()
    dump_data['file_date'] = get_last_modified_time(dump_file_path)
    print(f"file date: {dump_data['file_date']}")

    print(f"file {dump_file_path} found, read it:")
    c = 0
    # ---
    compare_file_date(dump_data['file_date'])
    # ---
    with bz2.open(dump_file_path, "r") as f:
        for line in f:
            line = line.decode("utf-8").strip("\n").strip(",")
            dump_data['done'] += 1
            # ---
