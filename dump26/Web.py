    ...
    for line in linesc:
        ...
        for pid, qids in claims.items():
            ...
            if pid not in tabs["properties"]:
                tabs["properties"][pid] = {"qids": {}, "lenth_of_usage": 0, "len_of_qids": 0, "len_prop_claims": 0}
...
def process_file_from_url(url):
    ...
    for i, entity_dict in enumerate(parse_lines_from_url(url), start=1):
        if i < skip_to:
            if i % dump_numbs == 0:
        ...
        if dump_numbs == len(lines):
            ...
            with open(done1_lines, "a", encoding="utf-8") as f:
                f.write(f"done: {i:,} {time.time() - tt[1]}\n")
        elif i % mem_nu == 0:
            print_memory(i)
